...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        apply_initial_random : bool = True,
        ...
        self.apply_initial_random = apply_initial_random
    ...
    def reset(
        ...
        *,
        ...
        if self.random_initial_state:   # if aggiunto da me sennò era solo l'else
            candidate_x = (VIEWPORT_W / SCALE) * 0.7
            initial_point = self.np_random.uniform(
                -candidate_x/2, candidate_x/2 )
            initial_x = VIEWPORT_W / SCALE / 2 + initial_point
        ...
        if self.apply_initial_random:   # if aggiunto da me
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                ...
                angle=(i * 0.05+initial_angle),     # aggiunto da me +initial_angle
                fixtures=fixtureDef(
...
def my_LunarLander(render_mode=None):
    env = TimeLimit(
        LunarLander(render_mode=render_mode, random_initial_state=True, apply_initial_random=False, enable_wind=False),
