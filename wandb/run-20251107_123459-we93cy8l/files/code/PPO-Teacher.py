...
class Args:
    ...
    # Cambia questa riga
    enviroment: Literal["LunarLander", "CarRacing", "Pusher-v5"] = "CarRacing"
...
def train():
    ...
    if args.enviroment in ['LunarLander', 'Pusher-v5']:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if agent.agent_type == 'Image':
                if step == 0 and iteration == 1:
                    input = agent.init_deque(get_screen(envs = None, screen = next_obs.cpu())).to(args.device)
                else:
                    input = agent.append_deque(get_screen(envs = None, screen = next_obs.cpu()), next_done.cpu()).to(args.device)
            else:
                input = next_obs.to(args.device)
            obs[step] = input
        ...
        wandb.log({"Teacher_training/chart/Average_reward": Running_Average, 'iteration' : iteration}, step=global_step)
        ...
        wandb.log({"iteration": iteration}, step=global_step)
